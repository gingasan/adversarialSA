...
class Bert(BertPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = self.Embeddings(config)
    ...
    class Attention(nn.Module):
        def __init__(self, config):
            super().__init__()
            self.self = Bert.SelfAttention(config)
            self.output = Bert.SelfOutput(config)
    ...
    class Layer(nn.Module):
        def __init__(self, config):
            ...
            self.seq_len_dim = 1
            self.attention = Bert.Attention(config)
            self.intermediate = Bert.Intermediate(config)
            self.output = Bert.Output(config)
    ...
    class Encoder(nn.Module):
        def __init__(self, config):
            super().__init__()
            self.layer = nn.ModuleList([Bert.Layer(config) for _ in range(config.num_hidden_layers)])
...
class BertForTokenClassification(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = Bert(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
...
class BertForMaskedLM(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.bert = Bert(config, add_pooling_layer=False)
        self.cls = self.OnlyMLMHead(config)
    ...
    class LMPredictionHead(nn.Module):
        def __init__(self, config):
            super().__init__()
            self.transform = BertForMaskedLM.PredictionHeadTransform(config)
    ...
    class OnlyMLMHead(nn.Module):
        def __init__(self, config):
            super().__init__()
            self.predictions = BertForMaskedLM.LMPredictionHead(config)
...
class AsaBert(Bert):
    ...
    class Adversary(nn.Module):
        def __init__(self, config):
            ...
            self.grl = AsaBert.GradientReversal()
    ...
    class Encoder(nn.Module):
        def __init__(self, config):
            super().__init__()
            self.layer = nn.ModuleList([Bert.Layer(config) for _ in range(config.num_hidden_layers)])
            self.adversary = nn.ModuleList([AsaBert.Adversary(config) for _ in range(config.num_hidden_layers)])
...
class AsaBertForSequenceClassification(BertForSequenceClassification):
    def __init__(self, config, tau=0.5):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.tau = tau

        self.bert = AsaBert(config, tau=tau)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
...
class AsaBertForMultipleChoice(BertForMultipleChoice):
    def __init__(self, config, tau=0.5):
        super().__init__(config)
        self.tau = tau

        self.bert = AsaBert(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, 1)
...
class AsaBertForTokenClassification(BertForTokenClassification):
    def __init__(self, config, tau=0.5):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.tau = tau

        self.bert = AsaBert(config, add_pooling_layer=False)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifier = nn.Linear(config.hidden_size, config.num_labels)
